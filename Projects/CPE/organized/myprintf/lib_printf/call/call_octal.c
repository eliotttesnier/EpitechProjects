...
int put_pos_mods_o(char *current_flag, int nbr, int nb_it)
{
...
int call_octal(char *current_flag, va_list list, int i)
{
    ...
    int nb_it = pfmy_getnbr(current_flag, 0) - my_len_octal(nbr);
    int neg_mods = 0;
    ...
    for (int j = 0; current_flag[j] != '\0'; j++) {
        if (current_flag[j] == '-') {
            neg_mods = 1;
            nb_it = pfmy_getnbr(current_flag, j + 1) - my_len_octal(nbr);
            put_neg_mods(current_flag, nbr, nb_it);
        }
    }
    if (neg_mods == 0)
        put_pos_mods_o(current_flag, nbr, nb_it);
}
