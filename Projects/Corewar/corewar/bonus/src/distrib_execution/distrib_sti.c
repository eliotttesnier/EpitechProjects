...
{
    int pc = vm->crt_prog->pc;
    int ind = (int)vm->map[(pc + 2) % MEM_SIZE];

    *off = *off + 1;
    return (vm->crt_prog->reg[ind - 1]);
}
...
static int get_third_value(map_t *vm, int *off)
{
    int pc = vm->crt_prog->pc;
    int byte = convert_to_binary(vm->map[(pc + 1) % MEM_SIZE]);
    int nb3 = (byte / 100) % 100;
...
int distrib_sti(map_t *vm)
{
    ...
    int val3 = get_third_value(vm, &off);

    exec_sti(vm, val1, val2, val3);
    return (calc_size_sti(vm) + 1);
}
