...
POneMap::POneMap(sf::Vector2f position, const std::string &texture_path)
    : AbstractEffectPlatform(position, texture_path)
{}

POneMap::POneMap(const JsonObject *conf) : AbstractEffectPlatform(conf)
{}

void POneMap::applyEffect()
{
    currentGame->getCurrentLevel()->reset();
}

bool POneMap::hit(sf::Sprite *sprite)
{
    float biggerX = 0.0f;
...
POneMap &POneMap::operator=(const POneMap &other)
{
    if (&other == this)
        return *this;
