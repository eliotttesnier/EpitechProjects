...
EThreeMap::EThreeMap(sf::Vector2f position, const std::string &texture_path)
    : AbstractEffectPlatform(position, texture_path)
{}

EThreeMap::EThreeMap(const JsonObject *conf) : AbstractEffectPlatform(conf)
{}

void EThreeMap::applyEffect()
{
    currentGame->getCurrentLevel()->reset();
}

bool EThreeMap::hit(sf::Sprite *sprite)
{
    float biggerX = 0.0f;
...
EThreeMap &EThreeMap::operator=(const EThreeMap &other)
{
    if (&other == this)
        return *this;
