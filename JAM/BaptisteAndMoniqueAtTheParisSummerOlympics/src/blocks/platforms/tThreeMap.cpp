...
TThreeMap::TThreeMap(sf::Vector2f position, const std::string &texture_path)
    : AbstractPlatform(position, texture_path)
{}

TThreeMap::TThreeMap(const JsonObject *json) : AbstractPlatform(json)
{}

bool TThreeMap::hit(sf::Sprite *sprite)
{
    float biggerX = 0.0f;
...
TThreeMap &TThreeMap::operator=(const TThreeMap &other)
{
    if (&other == this)
        return *this;
