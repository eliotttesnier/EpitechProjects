...
Test(cdup_command, already_at_root)
{
    ...
    cr_assert_eq(cdup_command(serv, client, args), 0);
    check_reply(pipe_fd[0], "550 Already at root directory\r\n");
...
// Valid directory
Test(cdup_command, valid_directory)
{
    server_t *serv = create_server();
    client_t *client = create_client();
    char *args = NULL;
