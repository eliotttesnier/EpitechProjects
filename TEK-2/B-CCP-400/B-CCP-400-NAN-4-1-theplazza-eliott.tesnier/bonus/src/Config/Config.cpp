 ...
 * @param useGui Whether to use the GUI interface
 */
Config::Config(float multiplier, int cooksPerKitchen, int regenerationTimeMs, bool useGui)
    : _multiplier(multiplier), _cooksPerKitchen(cooksPerKitchen),
    _regenerationTimeMs(regenerationTimeMs), _useGui(useGui)
{
}
...
/**
 * @brief Get whether to use GUI interface
 * @return bool True if GUI should be used
 */
bool Config::useGui() const
{
    return _useGui;
}
...
void Config::printUsage(const std::string &programName)
{
    std::cerr << ColorUtils::BOLD << "USAGE: " << ColorUtils::RESET
              << programName << " <multiplier> <cooks_per_kitchen> <regeneration_time_ms> [-d]"
              << std::endl;
    ...
    std::cerr << "  - " << ColorUtils::CYAN << "-d" << ColorUtils::RESET
              << ": optional flag to enable GUI interface" << std::endl;
}
...
Config Config::parseArguments(int argc, char **argv)
{
    ...
    return Config(multiplier, cooksPerKitchen, regenerationTimeMs, useGui);
}
