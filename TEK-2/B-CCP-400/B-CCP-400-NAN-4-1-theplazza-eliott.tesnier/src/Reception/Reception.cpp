...
void Reception::displayWelcomeMessage() const
{
    ...
    std::cout << "Enter your pizza order using the format: TYPE SIZE xN; [TYPE SIZE xN; ...]"
              << std::endl;
    std::cout << "Example: regina M x2; margarita L x1; americana S x3" << std::endl;
}
...
void Reception::displayHelp() const
{
    ...
    std::cout << "      TYPE: Regina, Margarita, Americana, Fantasia" << std::endl;
...
Pizza::PizzaOrder Reception::parsePizzaOrder(const std::string& orderItem) const
{
    ...
    if (std::regex_match(orderItem, matches, orderPattern)) {
        if (matches.size() == 4) {
            ...
            order.type = stringToPizzaType(typeStr);
            if (order.type == static_cast<Pizza::PizzaType>(0)) {
...
/**
 * @brief Convert a string to a PizzaType
 *
 * @param typeStr The pizza type as a string
 *
 * @return The corresponding PizzaType enum value, or 0 if invalid
 */
Pizza::PizzaType Reception::stringToPizzaType(const std::string& typeStr) const
{
    for (const auto& pair : Pizza::TYPE_NAMES) {
        if (pair.second == typeStr)
            return pair.first;
    }
    return static_cast<Pizza::PizzaType>(0);
}
...
{
    for (const auto& pair : Pizza::SIZE_NAMES) {
        if (pair.second == sizeStr)
            return pair.first;
    }
    return static_cast<Pizza::PizzaSize>(0);
}
