...
bool FifoIPCChannel::send(const std::string &message)
{
    ...
    fcntl(_writeFd, F_SETFL, oldFlags);
    std::cout << "[DEBUG][IPC][" << (_mode == Mode::PARENT ? "PARENT" : "CHILD") << "] sent: \"" << message << "\"" << std::endl;
    return true;
}
...
std::string FifoIPCChannel::receive()
{
    ...
    if (bytesRead != static_cast<ssize_t>(length)) {
        ...
        std::cerr << "Incomplete data read, got "
                  ...
                  << length
                  << " bytes" << std::endl;
        std::string partialMessage = message.substr(0, bytesRead);
        std::cout << "[DEBUG][IPC][" << (_mode == Mode::PARENT ? "PARENT" : "CHILD") << "] received: \"" << partialMessage << "\"" << std::endl;
        return partialMessage;
    }

    std::cout << "[DEBUG][IPC][" << (_mode == Mode::PARENT ? "PARENT" : "CHILD") << "] received: \"" << message << "\"" << std::endl;
    return message;
}
