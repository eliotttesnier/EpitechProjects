...
TOneMap::TOneMap(sf::Vector2f position, const std::string &texture_path)
    : AbstractPlatform(position, texture_path)
{}

TOneMap::TOneMap(const JsonObject *json) : AbstractPlatform(json)
{}

bool TOneMap::hit(sf::Sprite *sprite)
{
    float biggerX = 0.0f;
    ...
    for (std::size_t i = sprite->getGlobalBounds().top; i < sprite->getGlobalBounds().top + sprite->getGlobalBounds().height; i++) {
        for (std::size_t j = sprite->getGlobalBounds().left; j < sprite->getGlobalBounds().left + sprite->getGlobalBounds().width; j++) {
            ...
            if (position.y < _position.y || position.y > position.y + simpleBlockSize || position.y < _position.y + limit)
                return false;
        }
    }
    return true;
...
TOneMap &TOneMap::operator=(const TOneMap &other)
{
    if (&other == this)
        return *this;
