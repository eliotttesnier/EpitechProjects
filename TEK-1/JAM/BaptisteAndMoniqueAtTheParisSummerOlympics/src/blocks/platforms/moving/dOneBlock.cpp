...
DOneBlock::DOneBlock(sf::Vector2f start_pos, sf::Vector2f end_pos, float speed, const std::string &texture_path, MovingColor color)
    : AbstractMovingPlatform(start_pos, end_pos, speed, texture_path)
{
    _color = color;
}

DOneBlock::DOneBlock(const JsonObject *conf)
    : AbstractMovingPlatform(conf)
{}

bool DOneBlock::hit(sf::Sprite *sprite)
{
