...
PThreeMap::PThreeMap(sf::Vector2f position, const std::string &texture_path)
    : AbstractEffectPlatform(position, texture_path)
{}

PThreeMap::PThreeMap(const JsonObject *conf) : AbstractEffectPlatform(conf)
{}

void PThreeMap::applyEffect()
{
    currentGame->getCurrentLevel()->reset();
}

bool PThreeMap::hit(sf::Sprite *sprite)
{
    float biggerX = 0.0f;
...
PThreeMap &PThreeMap::operator=(const PThreeMap &other)
{
    if (&other == this)
        return *this;
